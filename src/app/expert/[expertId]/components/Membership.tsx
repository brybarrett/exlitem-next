import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import { Building2, GraduationCap } from 'lucide-react';
import { formatDateString } from '@/composables/dates';
import { Label } from '@/components/ui/label';
import { Switch } from '@/components/ui/switch';
import { useState } from 'react';
import apiClient from '@/lib/axios';
import { toast } from 'sonner';
interface MembershipSectionProps {
  membership: {
    membership_type: string;
    position_held: string;
    organization_name: string;
    start_date: string;
    end_date: string;
    is_current: boolean;
    uuid?: string;
    display_setting: string;
  }[];
  isOwner: boolean;
  isAdmin: boolean;
  profileType: string;
  handleActionLoading: (isLoading: boolean) => void;
  setShowUpgradeDialog: (show: boolean) => void;
}

export function MembershipSection({
  membership: initialMembership,
  isOwner,
  isAdmin,
  profileType,
  handleActionLoading,
  setShowUpgradeDialog
}: MembershipSectionProps) {
  const [membership, setMembership] = useState(initialMembership);
  const updateDisplaySetting = async (
    uuid: string,
    model: string,
    display_setting: string,
    originalSetting: string
  ) => {
    if ((profileType == 'Claimed' || profileType == 'Unclaimed') && !isAdmin) {
      setShowUpgradeDialog(true);
      return originalSetting;
    }
    handleActionLoading(true);
    try {
      const response = await apiClient.patch(`/experts/${model}/${uuid}/`, {
        display_setting: display_setting
      });
      if (response?.data?.display_setting) {
        toast.success(
          response?.data?.detail ||
            `${model} display setting updated successfully`
        );
        return response.data.display_setting;
      }
      return originalSetting;
    } catch (error: any) {
      toast.error(
        error?.response?.data?.detail ||
          `Failed to update ${model} display setting`
      );
      return originalSetting;
    } finally {
      handleActionLoading(false);
    }
  };
  if (!membership || membership.length === 0) {
    return (
      <Card>
        <CardHeader>
          <CardTitle className='text-xl font-semibold'>Memberships</CardTitle>
        </CardHeader>
        <CardContent>
          <div className='flex flex-col items-center justify-center py-8 text-center'>
            <Building2 className='text-muted-foreground mb-4 h-12 w-12' />
            <p className='text-muted-foreground text-sm'>
              No memberships available.
            </p>
          </div>
        </CardContent>
      </Card>
    );
  }

  return (
    <Card>
      <CardHeader>
        <CardTitle className='text-xl font-semibold'>Memberships</CardTitle>
      </CardHeader>
      <CardContent>
        <div className='space-y-6'>
          {membership.map((membership, index) => (
            <div key={index} className='flex items-start justify-between gap-2'>
              <div className='flex gap-4'>
                <div className='bg-muted flex h-12 w-12 shrink-0 items-center justify-center rounded-lg'>
                  <GraduationCap className='text-muted-foreground h-6 w-6' />
                </div>
                <div className='flex-1'>
                  <h4 className='font-medium'>{membership.membership_type}</h4>
                  <p className='text-muted-foreground text-sm'>
                    {membership.position_held}{' '}
                    {membership.organization_name
                      ? `at ${membership.organization_name}`
                      : ''}
                  </p>
                  <p className='text-muted-foreground text-sm'>
                    {formatDateString(membership.start_date)}
                    {membership.start_date &&
                      (membership.end_date || membership.is_current) && (
                        <span className='mx-2'>-</span>
                      )}
                    {membership.end_date
                      ? formatDateString(membership.end_date)
                      : membership.is_current
                        ? 'Present'
                        : ''}
                  </p>
                </div>
              </div>
              {(isOwner || isAdmin) && (
                <div key={membership.uuid} className='flex items-center gap-1'>
                  <Label
                    htmlFor={membership.uuid}
                    className='flex items-center gap-2'
                  >
                    Private
                  </Label>
                  <Switch
                    id={membership.uuid}
                    checked={membership.display_setting === 'Public'}
                    onCheckedChange={async (checked) => {
                      const newDisplaySetting = checked ? 'Public' : 'Private';
                      try {
                        const updatedSetting = await updateDisplaySetting(
                          membership.uuid!,
                          'expert_membership',
                          newDisplaySetting,
                          membership.display_setting
                        );
                        setMembership((prevMembership) =>
                          prevMembership.map((item) =>
                            item.uuid === membership.uuid
                              ? { ...item, display_setting: updatedSetting }
                              : item
                          )
                        );
                      } catch (error) {
                        // Error is already handled in updateDisplaySetting
                      }
                    }}
                  />
                  <Label
                    htmlFor={membership.uuid}
                    className='flex items-center gap-2'
                  >
                    Public
                  </Label>
                </div>
              )}
            </div>
          ))}
        </div>
      </CardContent>
    </Card>
  );
}
